...
public class WorkQueues {
    ...
    public void put(WebURL url) throws DatabaseException {
        ...
        //modified by sharat
        //crawl by either depth of link or last modified time of link.
        switch (CrawlerParameters.selectedPriorityOption)  {
            case DEPTHCRAWL:

                keyData[1] = (url.getDepth() > Byte.MAX_VALUE ? Byte.MAX_VALUE : (byte) url.getDepth());
                break;
            case LAST_MODIFIED_TIME_CRAWL:
                URLConnection url_time= null;
                try {

                    url_time = new URL(url.getURL()).openConnection();
                    Long last_modified_time = (Long.MAX_VALUE - url_time.getLastModified());

                    keyData[1] = last_modified_time.byteValue();
                    break;
